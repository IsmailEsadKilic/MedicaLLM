...
function Chat() {
  ...
  const saveEdit = async () => {
    try {
      const token = localStorage.getItem('token');
      await fetch(`/api/conversations/${editingId}/title`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const sendMessage = async (e) => {
    ...
    const chatsWithUserMessage = currentChats.map(c => 
      c.id === chatId 
        ? { ...c, messages: [...(c.messages || []), userMessage] }
        : c
    );
    setChats(chatsWithUserMessage);
    ...
    try {
      ...
      if (isFirstMessage) {
        fetch('/api/drugs/generate-title', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        ...
        .then(data => {
          if (data.title) {
            ...
            // Save to database
            fetch(`/api/conversations/${chatId}/title`, {
              method: 'PATCH',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
      ...
      const messages = chat?.messages || currentChat?.messages || [];
